...
shenron= pygame.image.load('shenron.png')
life = pygame.image.load('heart2.png')
gokuwin = pygame.image.load('gokuwins.png')
gokulose = pygame.image.load('gokulost.png')
...
def redrawGameWindow():
    ...
    for j in range(0,lives):
        win.blit(life, (10 + (j*64),10))
    
    if not(goblin.visible) and not(goblin2.visible) and man.x >= 600 and man.y == 410:  #if goblin dead and reached win point
        win.blit(shenron, (566,282))
        '''font1= pygame.font.SysFont("Stencil Std" , 70)
        text1= font1.render("Weehee! YOU WON!!!" , 1, (255,0,255))
        win.blit(text1, (350 -(text1.get_width()/2),200))'''
        win.blit(gokuwin,(150,0))
       
    if lives==0:
        '''font1= pygame.font.SysFont("Stencil Std" , 70, True)
        text1= font1.render("Awwwwhh! YOU LOSE!!!" , 1, (255,0,255))
        win.blit(text1, (350 -(text1.get_width()/2),200))'''
        win.blit(gokulose, (200,0))
...
bullets= []
goblin = enemy(250,415,64,64,500)
goblin2 = enemy(225,415,64,64,380)
...
while run:
    ...
    if man.hitbox[1] < goblin2.hitbox[1] + goblin2.hitbox[3] and man.hitbox[1] + man.hitbox[3] > goblin2.hitbox[1] and goblin2.visible: # man -> above the bottom of rect and below the top and if goblin is not dead
                if man.hitbox[0] < goblin2.hitbox[0] + goblin2.hitbox[2] and man.hitbox[0] + man.hitbox[2] > goblin2.hitbox[0]:
                    man.hit()
                    score -= 5
                    lives -= 1
    ...
    if shootloop > 0:
        shootloop += 1
    if shootloop > 7:  #basically it waits for bullet cool down for 7 frames
        shootloop = 0
    ...
    for bullet in bullets:
        ...
        if bullet.y - bullet.rad < goblin2.hitbox[1] + goblin2.hitbox[3] and bullet.y + bullet.rad > goblin2.hitbox[1] and goblin2.visible: # bullet -> above the bottom of rect and below the top
            if bullet.x - bullet.rad < goblin2.hitbox[0] + goblin2.hitbox[2] and bullet.x + bullet.rad > goblin2.hitbox[0]:        
                
                goblin2.hit();
                hitSound.play()
                score += 10
                bullets.pop(bullets.index(bullet))      #bullet gayab when goblin2 shot
    ...
    if not man.isJump:                  #allow jump if not jumping
        ...
        # Jump Code (10(initial) -10 count up -10 count down)  
        if man.jumpCount>=-10:
           if man.jumpCount>=0:
    ...
    if not(goblin.visible) and not(goblin2.visible) and man.x >= 600 and man.y == 410:
        i=0
        while i<1000:
            pygame.time.delay(10)
            i+=1
